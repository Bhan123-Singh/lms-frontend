 ...
 function Displaylectures(){
    ...
    const [currentVideo,setCurrentVideo]=useState(0);
     async function onLectureDelete(courseId,lectureId){
           ...
           await dispatch(deleteCourseLecture({
            courseId: courseId, lectureId : lectureId
           }));
           await dispatch(getAllCourses(courseId))
    }
    ...
    useEffect(()=>{
        ...
        dispatch(getAllCourses(state._id))
    ...
    return(
        <HomeLayout>
            <div className="flex flex-col gap-10 items-center justify-center min-h-[90vh] py-10 text-white mx-5%">
                ...
                {(lectures && lectures.length>0)?( <div className="flex justify-center gap-10 w-full">
                    {/* left section for playing videos and displaying course datails to admin */}
        ...
        <ul className="w-[28rem] p-2 rounded-lg shadow-[0_0_10px_black] space-y-4">
            <li className="font-semibold text-xl text-yellow-500 flex items-center justify-between">
                ...
                {role==='ADMIN' && (
                    <button onClick={()=> navigate('/course/addlecture',{state:{...state}})} className="btn-primary px-2 py-1 font-semibold text-sm">
                       Add new lecture 
                    </button>
                )}
            </li>
                ...
                return(
                   ...
                   {role === 'ADMIN' && (
                    ...
                    className="btn-accent px-2 py-1 font-semibold text-sm">
                        ...
                        className="btn-primary px-2 py-1 font-semibold text-sm">
                           Add new lecture 
                        </button>
                    )
                )}
